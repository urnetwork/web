...
operators: {
        ...
        roles: [
            ...
            { tag: '02', title: '发出真实需求信号', body: '运营商按其真实使用量以 alpha 计费。每一笔存入都会进入储备，作为一种有收入支撑的信号，验证方在为各池评分时会将其计入权重。' },
            { tag: '03', title: '指定支付分配',     body: '每个结算周期，运营商提交一份 Merkle 支付清单，将其池在各矿工之间进行分配。它指定分配方式，但从不进行托管——每个矿工直接从合约领取其份额。' },
            { tag: '04', title: '开始使用',         body: '注册一个网络运营商密钥，运行 /verify 服务器，并存入资金即可开始。在启动阶段，运营商准入由所有者把关。' }
        ],
    ...
    miners: {
        ...
        roles: [
            ...
            { tag: '04', title: '从排放中赚取',     body: '矿工的报酬来自子网的排放。在运营商的池内，你凭一份证明领取每次结算中的份额——这是一种低门槛的基线奖励，没有需要争夺的槽位，也没有需要销毁的东西。', href: '/docs/provider', linkLabel: '矿工文档' },
