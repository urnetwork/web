new (function () {
  ...
  self.notifyByJwtChanged = function () {
    let buttonElements = document.querySelectorAll(
      'div[data-bs-target="#dialog-connect"]',
    );
  ...
  self.apiRequest = function (method, path, body) {
    ...
    let p = fetch(request).then((response) => {
      ...
      if (typeof AppleID !== "undefined") {
        ...
        if (window.connectAppleIDSignInOnSuccess) {
          document.removeEventListener(
            ...
            window.connectAppleIDSignInOnSuccess,
          );
        }
        ...
        if (window.connectAppleIDSignInOnFailure) {
          document.removeEventListener(
            ...
            window.connectAppleIDSignInOnFailure,
          );
        }
      ...
      if (responseBody) {
        if ("network" in responseBody) {
          ...
          if (authAllowed.includes("password") && userAuth) {
            ...
            let message;
            if (
              authAllowed.includes("apple") &&
              authAllowed.includes("google")
            ) {
          ...
          self.mount.render(
            new connectSelf.DialogCreateNetworkAuthJwt(
              authJwtType,
              authJwt,
              userName,
            ),
    ...
    self.handleSubmitResponse = (responseBody) => {
      ...
      if (responseBody) {
        if ("error" in responseBody) {
          ...
          let suggestedUserAuth =
            error["suggested_user_auth"] || loginUserAuthElement.value;
    ...
    self.handleSubmitResponse = (responseBody) => {
      ...
      if (responseBody) {
        if ("error" in responseBody) {
          ...
          self.mount.render(
            new connectSelf.DialogCreateNetworkVerify(verificationUserAuth),
          );
  ...
  self.DialogPasswordReset = function (userAuth) {
    ...
    self.render = (container) => {
      ...
      const passwordResetUserAuthElement = self.element(
    ...
    self.submit = () => {
      ...
      const passwordResetButtonElement = self.element("password-reset-button");
      const passwordResetSpinnerElement = self.element(
    ...
    self.handleSubmitResponse = (responseBody) => {
      ...
      if (responseBody) {
        if ("error" in responseBody) {
          ...
          self.mount.render(
            new connectSelf.DialogPasswordResetAfterSend(responseUserAuth),
          );
        } else {
          let message = "Something went wrong. Please try again later.";
  ...
  self.DialogPasswordResetAfterSend = function (userAuth) {
    ...
    self.resend = () => {
      let passwordResetResendLinkElement = self.element(
        "password-reset-resend-link",
      );
      let passwordResetResendSpinnerElement = self.element(
        "password-reset-resend-spinner",
      );
    ...
    self.handleResendResponse = (responseBody) => {
      ...
      let passwordResetResendSentElement = self.element(
        "password-reset-resend-sent",
      );
  ...
  self.DialogPasswordResetComplete = function (resetCode) {
    ...
    self.render = (container) => {
      ...
      const passwordResetButtonElement = self.element("password-reset-button");
      const passwordResetPasswordElement = self.element(
        "password-reset-password",
      );
      const passwordResetPasswordConfirmElement = self.element(
        ...
        if ("error" in responseBody) {
          ...
          if ("reset_code_error" in error) {
            ...
            errorElement.innerHTML =
              'Reset code expired. <a href="/connect/password-reset">Get a new code</a>.';
            errorElement.classList.remove("d-none");
          } else {
            let message = error["message"];
  ...
  self.NetworkNameValidator = function (
    ...
    createNetworkNameElement.addEventListener("input", (event) => {
      if (createNetworkNameElement.value == "") {
        ...
        !self.networkEdited &&
        document.activeElement == createNetworkNameElement
      ) {
        self.networkEdited = true;
      }

      let normalNetworkName = normalizeNetworkName(
        createNetworkNameElement.value,
      );
      if (normalNetworkName != createNetworkNameElement.value) {
        ...
        createNetworkNameElement.setSelectionRange(
          selectionStart,
          selectionEnd,
        );
      }
    ...
    self.debounceValidateNetworkName = (networkName) => {
      ...
      self.validateTimeout = setTimeout(() => {
        (self.validateNetworkName(networkName), 500);
      });
    };
    ...
    self.handleValidateNetworkNameResponse = (responseBody) => {
      ...
      if (!responseBody) {
        ...
        validateError =
          "Not available. Must be at least 3 characters different than an existing network. Make it unique or longer.";
      }
  ...
  self.DialogCreateNetworkAuthJwt = function (authJwtType, authJwt, userName) {
    ...
    self.render = (container) => {
      connectSelf.renderCreateNetworkAuthJwt(
        container,
        self.id,
        authJwtType,
        authJwt,
      ...
      const createNetworkNameSpinnerElement = self.element(
        "create-network-name-spinner",
      );
      const createNetworkNameErrorElement = self.element(
        "create-network-name-error",
      );
      const createNetworkNameAvailableElement = self.element(
        "create-network-name-available",
      );
      ...
      if (responseBody) {
        if ("error" in responseBody) {
          ...
          if (error["user_auth_conflict"]) {
            createUserAuthErrorElement.innerHTML =
              'This email or phone number is already taken. <a href="/connect">Sign in</a>.';
          ...
          if ("network_name_message" in error) {
            createNetworkNameErrorElement.textContent =
              error["network_name_message"];
            createNetworkNameErrorElement.classList.remove("d-none");
          } else {
            createNetworkNameErrorElement.classList.add("d-none");
          }
  ...
  self.DialogCreateNetwork = function (userAuth) {
    ...
    self.render = (container) => {
      ...
      createPasswordElement.addEventListener("input", (event) => {
        ...
        if (validatePasswordEntropy(password)) {
          ...
          createPasswordErrorElement.textContent =
            "Password must be at least 65 bits of entropy (at least 12 characters, mixed case, mixed alphanumeric and non-alphanumeric).";
    ...
    self.submit = () => {
      ...
      const balanaceCodeElement = self.element("create-balance-code");
      ...
      balanaceCodeElement.disabled = true;
      createSpinnerElement.classList.remove("d-none");
      ...
      let balanceCode = balanaceCodeElement.value;
      let requestBody = {
        user_name: userName,
        user_auth: userAuth,
        password: password,
    ...
    self.handleSubmitResponse = (responseBody) => {
      ...
      balanaceCodeElement.disabled = false;
      createSpinnerElement.classList.add("d-none");
      ...
      if (responseBody) {
        if ("error" in responseBody) {
          ...
          if (error["user_auth_conflict"]) {
            createUserAuthErrorElement.innerHTML =
              'This email or phone number is already taken. <a href="/connect">Sign in</a> or <a href="/connect/password-reset">reset your password</a>.';
            createUserAuthErrorElement.classList.remove("d-none");
  ...
  self.DialogComplete = function () {
    ...
    self.render = (container) => {
      ...
      const copyAuthCodeManualCodeCopyButtonElement = self.element(
        "copy-auth-code-manual-code-copy",
      );
      ...
      copyAuthCodeManualCodeCopyButtonElement.addEventListener(
        "click",
        (event) => {
      ...
      const preferencesProductUpdateElement = self.element(
    ...
    self.handleCopyAuthCodeResponse = (responseBody) => {
      ...
      const copyAuthCodeErrorElement = self.element("copy-auth-code-error");
      const copyAuthCodeManualElement = self.element("copy-auth-code-manual");
      const copyAuthCodeManualCodeElement = self.element(
        "copy-auth-code-manual-code",
      );

      const copyAuthCodeTextElement = self.element("copy-auth-code-text");
      const copyAuthCodeTextCopiedElement = self.element(
        "copy-auth-code-text-copied",
      );
    ...
    self.copyAuthCodeManual = () => {
      ...
      const copyAuthCodeManualCodeCopyElement = self.element(
        "copy-auth-code-manual-code-copy",
      );
      ...
      navigator.clipboard
        .writeText(copyAuthCodeManualCodeElement.value)
        .then(() => {
    ...
    self.handleLaunchAppResponse = (responseBody, appRoute, windowRef) => {
      ...
      if (!responseBody) {
        ...
        let url = connectSelf.serviceUrl(
          "app",
          `${appRoute}${appRoute.includes("?") ? "&" : "?"}auth_code=${responseBody["auth_code"]}`,
        );
        if (windowRef) {
    ...
    self.submit = () => {
      ...
      const feedbackNeedCollaborateElement = self.element(
        "feedback-need-collaborate",
      );
      const feedbackNeedAppControlElement = self.element(
        "feedback-need-app-control",
      );
      const feedbackNeedBlockDataBrokersElement = self.element(
        "feedback-need-block-data-brokers",
      );
      const feedbackNeedBlockAdsElement = self.element(
        "feedback-need-block-ads",
      );
      const feedbackNeedFocusElement = self.element("feedback-need-focus");
      const feedbackNeedConnectServersElement = self.element(
        "feedback-need-connect-servers",
      );
      const feedbackNeedRunServersElement = self.element(
        "feedback-need-run-servers",
      );
      const feedbackNeedPreventCyberElement = self.element(
        "feedback-need-prevent-cyber",
      );
      const feedbackNeedAuditElement = self.element("feedback-need-audit");
      const feedbackNeedZeroTrustElement = self.element(
        "feedback-need-zero-trust",
      );
      const feedbackNeedVisualizeElement = self.element(
        "feedback-need-visualize",
      );
  ...
  self.renderInitial = function (container, id, nonce) {
    ...
    if (connectSelf.isAndroid() || connectSelf.isChromeOs()) {
      storeLink =
        "https://play.google.com/store/apps/details?id=com.bringyour.network";
    } else if (connectSelf.isApple()) {
      storeLink = "https://apps.apple.com/us/app/urnetwork/id6741000606";
    } else {
      storeLink = "/c?guest";
  ...
  self.renderCreateNetworkAuthJwt = function (
    container,
    id,
    authJwtType,
    authJwt,
