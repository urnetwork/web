...
const BulkDeleteForm: React.FC<BulkDeleteFormProps> = ({ clients, onClientsRemoved }) => {
  const { token } = useAuth();
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      for (let i = 0; i < clientsToDelete.length; i++) {
        ...
        try {
          ...
          if (response.error) {
            ...
            console.error(`Failed to delete client ${client.client_id}:`, response.error.message);
          } else {
      ...
      if (deletedClientIds.length > 0) {
        ...
        toast.success(`Successfully removed ${deletedClientIds.length} offline clients`);
      }
  ...
  const dayOptions = [
    { value: 0, label: 'Offline Only (Any Time)', description: 'Remove all offline clients regardless of when they were last seen' },
    { value: 1, label: '1+ Days Offline', description: 'Remove clients offline for more than 1 day' },
    { value: 7, label: '7+ Days Offline', description: 'Remove clients offline for more than 7 days (recommended)' },
    { value: 30, label: '30+ Days Offline', description: 'Remove clients offline for more than 30 days' },
  ];
      ...
      <div className="max-w-2xl">
        <div className="mb-6 p-4 bg-yellow-900/30 border border-yellow-700/50 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-yellow-300">Bulk Delete Warning</span>
          </div>
          <p className="text-xs text-yellow-200 mb-2">
            This will permanently remove multiple clients from your network. Connected clients will never be deleted.
          </p>
          <div className="text-xs text-yellow-300">
            <strong>Safety Features:</strong>
            <ul className="list-disc list-inside mt-1 space-y-1">
              <li>Online/connected clients are automatically protected</li>
              <li>Only offline clients past the selected time threshold are removed</li>
              <li>Each deletion is processed individually with error handling</li>
            </ul>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div className="bg-green-900/30 p-3 rounded border border-green-700">
                ...
                <div className="text-lg font-semibold text-green-200">
                  {clients.filter(c => c.connections && c.connections.length > 0).length}
                </div>
              </div>
              <div className="bg-red-900/30 p-3 rounded border border-red-700">
                <div className="text-red-300">Will Be Deleted</div>
                <div className="text-lg font-semibold text-red-200">{clientsToDelete.length}</div>
              </div>
            </div>
            
            {clientsToDelete.length > 0 && (
              <div className="mt-4 p-3 bg-gray-800 rounded border border-gray-600">
                <div className="text-xs text-gray-400 mb-2">Sample clients to be deleted:</div>
      ...
      >
        <div className="space-y-4">
          <p className="text-gray-300">
            Are you sure you want to delete <span className="font-medium text-white">{clientsToDelete.length}</span> offline clients?
          </p>
          
          <div className="bg-red-900/50 p-3 rounded-lg border border-red-700">
            <p className="text-sm text-red-300 font-medium mb-2">⚠️ This action will:</p>
            <ul className="text-sm text-red-200 space-y-1">
              <li>• Permanently remove {clientsToDelete.length} clients from your network</li>
          ...
          <div className="bg-blue-900/50 p-3 rounded-lg border border-blue-700">
            <p className="text-sm text-blue-300">
              <strong>Criteria:</strong> Clients offline for {selectedDays === 0 ? 'any amount of time' : `${selectedDays}+ days`}
            </p>
          </div>
