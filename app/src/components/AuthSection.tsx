...
const AuthSection: React.FC = () => {
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (userAuth.trim() && password.trim()) {
      const response = await loginWithPassword(userAuth.trim(), password.trim());
      
      if (response.verification_required) {
        // Handle verification required case - could show a verification form
        console.log('Verification required for:', response.verification_required.user_auth);
      }
    }
  };
        ...
        <div className="p-6 bg-gray-800">
          ...
          {activeTab === 'password' && (
            <form onSubmit={handlePasswordSubmit}>
              <div className="mb-4">
                <label htmlFor="userAuth" className="block text-sm font-medium text-gray-300 mb-2">
              ...
              <button
                type="submit"
                disabled={isLoading || !userAuth.trim() || !password.trim()}
                className={`w-full py-3 px-4 rounded-lg font-medium text-white transition-all duration-200 transform ${
                  isLoading || !userAuth.trim() || !password.trim()
                    ? 'bg-gray-600 cursor-not-allowed' 
                    : 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 hover:scale-105 shadow-lg hover:shadow-xl'
                }`}
              >
